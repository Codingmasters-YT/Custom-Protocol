...
int __cdecl wmain(int argc, wchar_t* argv[], wchar_t* envp[])
{
    ...
    WCHAR* wString = HeapAlloc(hProcessHeap, HEAP_ZERO_MEMORY, (nArgLength + 8) * sizeof(WCHAR)),
        * wBuffer = wString;
    wprintf(L"Argument passed to the Command Prompt :\n%s\n\n", argv[1]);
    if (wString == NULL) return 1;

    lstrcpyW(wString, argv[1]);
    if (wString[nArgLength - 1] == L'/')wString[nArgLength - 1] = 0;

    while (*wBuffer++ != 0) {
        static BOOL protocolPass = FALSE,
            quotesOpen = FALSE;
        if (!protocolPass) nOffset++;
        switch (*wBuffer) {
        case L'/': {
            if (!protocolPass) {
                protocolPass = TRUE;
        ...
        case L'%': {
            ...
            WCHAR nChar = (WCHAR)wcstoul(hexNumber, &cumRag, 16);
    ...
    wprintf(L"Confirm pass-through? [y/n] ");

    wscanf_s(L"%c", &bConsent, 1);
    if (bConsent != 0 && toupper(bConsent) != L'Y') return 1;

    WCHAR* wNewString = HeapAlloc(hProcessHeap, HEAP_ZERO_MEMORY, (nArgLength + 8) * sizeof(WCHAR));

    lstrcatW(wNewString, L"/c");
    ...
    CreateProcessW(
        L"C:\\Windows\\system32\\cmd.exe", wNewString,
        NULL,
        NULL,
        0,
